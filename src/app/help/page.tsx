    ...
    <main id={styles.main} onClick={( e: any)=> !CheckIncludes(e, 'menu') && !CheckIncludes(e, 'menu div') && !CheckIncludes(e, 'menu button') && !CheckIncludes(e, 'menu span') && RemoveAllClass( styles.inView , 'menu' )}>
      <div className={styles.main}>
        ...
        <div className={styles.default_bg}>
          ...
          <h3 className={error && 'text-[var(--error)!important]'}>{!isLoading ? error ? 'Error occured'  : (filters.trim() !== '' && help.filter(( h:any )=> h.title.includes(filters.trim()))) ? 'Search term does not exist' : 'Search help titles' : 'Getting help pages ...'}
          <p>{!isLoading ? error ? 'Could not get help pages' : (filters.trim() !== '' && help.filter(( h:any )=> h.title.includes(filters.trim()))) ? 'Help title does not exist on Cod-en. Try researching titles' : 'Discover more on Cod-en from our help pages.' : 'Loading help pages data.'}</p></h3>
          {(error || isLoading) && <section className='pt-6 w-full'><button className='flex gap-2.5 items-center justify-center rounded-[25px!important] mx-auto hover:bg-[#fc52524d]' style={{border: 'solid 2px', padding: '5px 23px', fontSize: '16px', color: 'var(--error)', borderColor: 'var(--error)', backgroundImage: 'linear-gradient( to right, #fc52524d, transparent'}} disabled={isLoading} onClick={()=>setRefresh(( prev : any)=> !prev)}>{Refreshsvg()} Refresh</button></section>}
        </div>
      </div>
